...
// BlankAppOptions is an empty implementation of servertypes.AppOptions
type BlankAppOptions struct{}

// Get implements the AppOptions interface.
func (ao BlankAppOptions) Get(key string) interface{} {
	return nil
}
...
func Setup(t *testing.T, opts ...wasmkeeper.Option) *WasmApp {
	...
	var logger log.Logger = log.NewLogger(os.Stdout, logLevel)
	...
	//////////////////////////////////////
	// Here we define a simple baseAppOption that sets a custom AnteHandler.
	// You might not need this if you don't have specific ante handler requirements.
	// anteHandlerOpt := func(bApp *baseapp.BaseApp) {
	// 	bApp.SetAnteHandler(customAnteHandler)
	// }

	// // Example baseAppOption that sets a custom init chainer
	// initChainerOpt := func(bApp *baseapp.BaseApp) {
	// 	bApp.SetInitChainer(customInitChainer)
	// }

	// // Combine all options into a slice
	// baseAppOptions := []func(*baseapp.BaseApp){anteHandlerOpt, initChainerOpt}
	...
	bApp.SetAnteHandler(func(ctx sdk.Context, tx sdk.Tx, simulate bool) (sdk.Context, error) {
		// Implement your custom ante handler logic here
		return ctx, nil
	})

	// Define and apply any custom init chainer if necessary
	// This is an example. Replace or remove if not applicable to your setup.
	// bApp.SetInitChainer(func(ctx sdk.Context, req abci.RequestInitChain) abci.ResponseInitChain {
	// 	// Implement your custom init chainer logic here
	...
	*/
	...
	app.ScopedWillKeeper = scopedWillKeeper
