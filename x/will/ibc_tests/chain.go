...
func NewTestChain(t *testing.T, coord *ibcgotesting.Coordinator, chainID string) *ibcgotesting.TestChain {
	...
	// acc := &willtypes.EthAccount{
	// 	BaseAccount: baseAcc,
	// 	CodeHash:    common.BytesToHash(evmtypes.EmptyCodeHash).Hex(),
	// }
	acc := "w3ll1p0k8gygawzpggzwftv7cv47zvgg8zaun5xucxz"

	amount := sdk.TokensFromConsensusPower(1, willtypes.PowerReduction)

	balance := banktypes.Balance{
		// Address: acc.GetAddress().String(),
		Address: acc,
		// Coins:   sdk.NewCoins(sdk.NewCoin(utils.BaseDenom, amount)),
		Coins: sdk.NewCoins(sdk.NewCoin("w3ll", amount)),
	}
